...
const Landing: React.FC = () => {
    const brands: Array<any> = [gucci,ck,adidas,nike,dg,zara]

    useEffect(() => {
        const listener = (): void => {
            var a: number = document.getElementsByClassName('landing__photo')[0].clientHeight;
            var b: number = document.getElementsByClassName('landing__advantages')[0].clientHeight;
            var c: number = document.getElementsByClassName('landing__details')[0].clientHeight;
            var d: number = document.getElementsByClassName('landing__brands')[0].clientHeight / 2;
            var scrollTop: number = (a+b+c+d+150);
        
            if (Math.round(window.pageYOffset) >= scrollTop) {
                var blocks: NodeListOf<Element> = document.querySelectorAll('.landing__counter > div > span');
